...
# install_debian.sh
# Installation and Uninstallation script for GenXPassword on Debian-based systems
...
install_dependencies() {
    ...
    # Check for Qt6 base libraries
    if ! dpkg -l | grep -q qt6-base; then
        echo "Qt6 base libraries not found. Installing..."
        apt-get install -y qt6-base-dev
    else
        echo "Qt6 base libraries are already installed."
    fi
...
install_icons() {
    ...
    if [ -f "$ICON_SRC_ICO" ]; then
        magick convert "$ICON_SRC_ICO" -resize 256x256 "$APP_NAME.png"
